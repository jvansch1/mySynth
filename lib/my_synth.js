...
masterGainNode.gain.value = 0.02;
...
function changeOverDriveType() {
  var newType = document.getElementById('drive-type').value;
  var newTypeNumber = parseInt(newType);
  _tuna.overdrive.algorithmIndex = newTypeNumber;
}
...
function bypassFilter() {
  ...
  if (newStatus) {
    _tuna.filter.bypass = 0;
  } else {
    _tuna.filter.bypass = 1;
  }
};
...
function createKey(note, freq, keyId) {
  ...
  $(document).on("keydown", function (e) {
    ...
    key.addClass('red');
    if (!fired) {
      var oscs = playTone($(key[0]).attr("data-frequency"));
      if (!oscs) return null;
